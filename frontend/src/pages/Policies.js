...
function Policies() {
  ...
  const loadPolicies = async () => {
    const res = await axios.get("http://localhost:5000/api/access/policies");
    setPolicies(res.data.result.data_array || []);
  };

  const deletePolicy = async (email, schema, table) => {
    await axios.post("http://localhost:5000/api/access/delete-policy", {
      email,
      schema,
      table
    });
  ...
  const togglePolicy = async (email, schema, table, active) => {
    await axios.post("http://localhost:5000/api/access/toggle-policy", {
      email,
      schema,
      table,
      active
