...
router.post("/request", async (req, res) => {
  try {
    ...
    const sql = `INSERT INTO workspace.governance.user_requests 
...
router.get("/history/:user", async (req, res) => {
  try {
    ...
    const sql = `SELECT * FROM workspace.governance.user_requests 
...
router.post("/login", async (req, res) => {
  try {
    ...
    const sql = `SELECT * FROM workspace.governance.users 
      WHERE email='${email}' AND password='${password}'`;
    const data = await runQuery(sql);

    if (data.result && data.result.data_array && data.result.data_array.length > 0) {
      const user = data.result.data_array[0];
      
      // Get department also
      const deptSql = `SELECT department FROM workspace.governance.access_policies 
        WHERE user_email='${email}' LIMIT 1`;
      const deptData = await runQuery(deptSql);
      
      let department = null;
      if (deptData.result && deptData.result.data_array && deptData.result.data_array.length > 0) {
...
router.post("/register", requireAdmin, async (req, res) => {
  try {
    ...
    const sql = `INSERT INTO workspace.governance.users 
...
router.get("/all-users", requireAdmin, async (req, res) => {
  try {
    const sql = `SELECT u.email, u.password, u.role, p.department 
      FROM workspace.governance.users u 
      LEFT JOIN workspace.governance.access_policies p ON u.email = p.user_email 
      ORDER BY u.email`;
    const data = await runQuery(sql);
    res.send(data);
  } catch (error) {
...
router.post("/delete-user", requireAdmin, async (req, res) => {
  try {
    ...
    await runQuery(`DELETE FROM workspace.governance.users WHERE email='${email}'`);
    await runQuery(`DELETE FROM workspace.governance.access_policies WHERE user_email='${email}'`);
...
router.post("/update-role", requireAdmin, async (req, res) => {
  try {
    ...
    const sql = `UPDATE workspace.governance.users 
      SET role='${role}' 
      WHERE email='${email}'`;
    await runQuery(sql);
...
router.post("/update-department", requireAdmin, async (req, res) => {
  try {
    ...
    const sql = `UPDATE workspace.governance.access_policies 
...
router.get("/policies", requireAdmin, async (req, res) => {
  try {
    const sql = `SELECT * FROM workspace.governance.access_policies 
...
router.post("/delete-policy", requireAdmin, async (req, res) => {
  try {
    ...
    const sql = `DELETE FROM workspace.governance.access_policies 
      WHERE user_email='${email}' 
      AND schema_name='${schema}' 
      AND table_pattern='${table}'`;
    await runQuery(sql);
    res.send({ success: true, message: "Policy deleted" });
  } catch (error) {
    res.status(500).send({ error: error.message });
  }
});
...
router.post("/toggle-policy", requireAdmin, async (req, res) => {
  try {
    ...
    const sql = `UPDATE workspace.governance.access_policies 
      ...
      AND table_pattern='${table}'`;
    await runQuery(sql);
    res.send({ success: true, message: "Policy updated" });
  } catch (error) {
    res.status(500).send({ error: error.message });
  }
});
...
router.get("/resources/:email", async (req, res) => {
  try {
    ...
    const sql = `SELECT department FROM workspace.governance.access_policies 
      WHERE user_email='${email}' LIMIT 1`;
    const result = await runQuery(sql);

    if (!result.result || !result.result.data_array || result.result.data_array.length === 0) {
      return res.send({ catalogs: [], schemas: [], tables: [] });
    }
...
router.get("/my-requests/:email", async (req, res) => {
  try {
    const email = req.params.email;
    const sql = `SELECT catalog, schema, table, access, status 
...
router.get("/my-access/:email", async (req, res) => {
  try {
    ...
    const tableSql = `SHOW TABLES IN workspace.${department}`;
    const tables = await runQuery(tableSql);
    
    res.send({ department, tables: tables.result.data_array });
  } catch (error) {
    res.status(500).send({ error: error.message });
  }
});
...
router.get("/table-data/:email/:table", async (req, res) => {
  try {
    ...
    const department = deptResult.result.data_array[0][0];
    const sql = `SELECT * FROM workspace.${department}.${table} LIMIT 100`;
    const data = await runQuery(sql);
...
router.get("/my-access-links/:user", async (req, res) => {
  ...
  try {
    ...
    const sql = `
      SELECT catalog_name, schema_name, table_pattern
...
router.get("/my-access/:user", async (req, res) => {
